...
<div>
	...
	<div>
		<c:if test="${startPage > 1 }">
			<a href = "<c:url value = '/seller/reservation/list?mem_id=${sessionScope.mem_id }&pageNum=${startPage - 1 }&checkin=${checkin }&checkout=${checkout }&field=${field }&keyword=${keyword }'/>"><span style = "color:skyblue;">[이전]</span></a>
		</c:if>
		<c:forEach var = "i" begin = "${startPage }" end = "${endPage }">
			<c:choose>
				<c:when test="${i == pageNum }">
					<a href = "<c:url value = '/seller/reservation/list?mem_id=${sessionScope.mem_id }&pageNum=${i }&checkin=${checkin }&checkout=${checkout }&field=${field }&keyword=${keyword }'/>"><span style = "color:skyblue;">[${i }]</span></a>
				</c:when>
				<c:otherwise>
					<a href = "<c:url value = '/seller/reservation/list?mem_id=${sessionScope.mem_id }&pageNum=${i }&checkin=${checkin }&checkout=${checkout }&field=${field }&keyword=${keyword }'/>"><span style = "color:gray;">[${i }]</span></a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		<c:if test="${endPage < pageCnt }">
			<a href = "<c:url value = '/seller/reservation/list?mem_id=${sessionScope.mem_id }&pageNum=${endPage + 1 }&checkin=${checkin }&checkout=${checkout }&field=${field }&keyword=${keyword }'/>"><span style = "color:skyblue;">[다음]</span></a>
		</c:if>
	</div>
</div>