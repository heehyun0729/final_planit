				...
				<div class="text-left">
					...
					<table class="table">
						...
						<c:forEach var="vo" items="${list}" varStatus="status">
							<tr>
								...
								<td><a
									href="${pageContext.request.contextPath }/admin/roomDetail?accom_num=${accom_num }&room_num=${vo.room_num}">${vo.room_type}</a></td>
								<td><c:forEach var="img" items="${vo.room_images}">
										<c:if test="${not empty img.roomImg_orgImg}">
											...
											<a
												href="${pageContext.request.contextPath }/admin/roomImageDelete?accom_num=${accom_num}&roomImg_num=${img.roomImg_num }"><i
												class="fas fa-trash" style="color: #339af0;"></i></a>
										</c:if>
									</c:forEach>
									<form method="post" id="form12"
										action="<c:url value='/admin/roomImageInsert'/>?accom_num=${accom_num}"
							...
							<ul class="pagination">
								<li class="page-item"><c:choose>
										<c:when test="${pu.startPageNum  > 5 }">
											<a
												href="${pageContext.request.contextPath }/admin/roomList?pageNum=${pu.startPageNum - 1}&field=${field}&keyword=${keyword}&accom_num=${accom_num}"
									...
									<c:choose>
										<c:when test="${pu.pageNum==i }">
											<li class="page-item active"><a
												href="${pageContext.request.contextPath }/admin/roomList?pageNum=${i}&accom_num=${param.accom_num}"
								...
								<li class="page-item"><c:choose>
										<c:when test="${pu.endPageNum < pu.totalPageCount}">
											<a
												href="${pageContext.request.contextPath }/admin/roomList?pageNum=${pu.endPageNum + 1}&field=${field}&keyword=${keyword}&accom_num=${accom_num}"
					...
					<div>
						<form method="post" id="form9"
							action="<c:url value='/admin/roomList?accom_num=${param.accom_num}'/>">
					...
					<br /> <a
						href="<c:url value='/admin/roomInsert?accom_num=${param.accom_num }'/>"
