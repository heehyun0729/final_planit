...
<section class="contact_area section_gap">
    <div class="container">
		<div class="row d-flex">
      		<div class = "p-2"><h2 class="title title_color">${avo.accom_name }</h2></div>
      		<div class = "p-2 ml-auto">
      			<a href = "<c:url value='/reservation/accomList?pageNum=${i }&keyword=${keyword }&checkin=${checkin}&checkout=${checkout }&cnt=${cnt }'/>" class="genric-btn success-border circle">목록</a>
      		</div>
      	</div>
      	<div class="row d-flex">
      		<div class = "p-2">
      			<p><i class="fas fa-map-marker-alt"></i> ${avo.accom_addr}</p>
      		</div>
      		<div class = "p-2">
      			<a href="javascript:showMap('${avo.accom_addr }','${avo.accom_name }')" class = "genric-btn primary-border small radius">지도보기</a>
      		</div>
      		<!-- 지도 모달 -->
			<div id="accomMapModal" class="modal fade" role="dialog">
				<div class="modal-dialog" role="document" style="max-width: 100%;">
					<div class="modal-content">
						<div class="modal-header">${avo.accom_name }의 위치</div>
		                        ...
		                        <div id="mc_embed_signup">
		                           <div class="input-group">
				                        <span class="input-group-btn">
		                       			...
		                       			<input type="number" id="rsvnCnt" min="1" name="cnt" value="${cnt }" class="form-control single-input" style = "width: 100px;">
		                               <span class="btn sub-btn disabled">명</span>
				                        <span class="input-group-btn">
				                            <button type="button" class="btn" onclick="increaseCnt()" style = "padding-left:0;">
				...
				<div class="col-lg-12">
					<blockquote class="generic-blockquote" style = "margin-bottom: 30px;">
						<div class="row" style = "margin-bottom: 10px;">
							<div class="col-lg-12" onclick="javascript:openRoomDialog(${rvo.room_num})" style = "cursor:pointer;">
								<span class = "sec_h4">${rvo.room_type }</span><span>(${rvo.room_capa }인실)</span>
								<hr>
							</div>
						</div>
						<div class="row">
							<div class="col-lg-8 d-flex" onclick="javascript:openRoomDialog(${rvo.room_num})" style = "cursor:pointer;padding-left: 0px;">
								<div class="col-md-3 p-2" style = "padding-left: 0px;">
									<img  class="img-thumbnail" src="<c:url value='/resources/uploadRoom/${rvo.room_images[0].roomImg_saveImg}'/>" style = "max-height: 150px;overflow: hidden;">
								</div>
								<div class="col-md-9 mt-sm-20 left-align-p p-2" style = "overflow: auto;">
									<p>${rvo.room_comm }</p>
								</div>
							</div>
							<div class="col-lg-4 d-flex justify-content-end" style = "padding-top: 50px;">
								<div class="col-md-7 p-2 text-center">
									<span>1박 가격</span>
									<h3 style = "color: #52C5FD;">${rvo.room_price }원</h3>
								</div>
								<div class="col-md-5 p-2 text-right">
									<a class="button_hover theme_btn" onclick="javascript:openRsvnDialog(${rvo.room_num})" style = "color: white;cursor:pointer;margin-top:5px;">예약하기</a>
								</div>
							</div>
						</div>
					</blockquote>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="text-center">
							...
							<p>
								<span style = "font-size:20px;color:#ff5252;" id="roomName"></span><span id="roomCapa"></span>
							</p>
						</div>
						<form method="post" action="<c:url value='/reservation/book'/>">
							...
							<div class="form-inline d-flex" style = "margin-bottom: 20px;">
								<input type="text" style = "width: 120px;" id="rsvnCheckinDatepicker" name="checkin" value="${checkin }" class = "form-control single-input p-2">
								<span class = "p-2">&nbsp;~&nbsp;</span>
								<input type="text" style = "width: 120px;" id="rsvnCheckoutDatepicker" name="checkout" value="${checkout }" class = "form-control single-input p-2">
							</div>
							<h6>인원</h6>
							<div class = "row" style = "margin-bottom: 20px;width:150px;padding-left: 16px;">
								<select id="rsvnDialogCnt"  name="cnt" class="form-control single-input nice-select"></select>
							</div>	
							<div class="text-center" style = "margin-bottom: 20px;padding-left: 16px;">
								<p id="rsvnMsg"></p>
								<div>
									<span>총</span> <span id="roomPrice" style = "font-size:20px;color:#ff5252;">0</span>원
								</div>
							</div>
							<div class="text-center">
								<input type="submit" id="btnPay" class="genric-btn primary circle" value="결제하기">
							</div>
						</form>
					</div>
				</div>
		...
		<ul class="nav nav-tabs" id="accomTabs" role="tablist">
		  <li class="nav-item">
		    <a class="nav-link active" id="accomReview-tab" data-toggle="tab" href="#accomReview" role="tab" aria-selected="true">후기</a>
		  </li>
		  <li class="nav-item">
		    <a class="nav-link" id="accomQna-tab" data-toggle="tab" href="#accomQna" role="tab" aria-selected="false">문의</a>
		  </li>
		</ul>
