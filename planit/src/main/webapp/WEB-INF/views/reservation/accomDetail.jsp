...
<div>
	...
	<div>
		...
		<p>${avo.accom_addr}
			<a
				href="javascript:showMap('${avo.accom_addr }','${avo.accom_name }')">지도
				보기</a>
		</p>
	</div>
	<!-- 지도 모달 -->
	...
	<div id="roomSearch">
		<form method="post"
			action="<c:url value = '/reservation/detail?accom_num=${avo.accom_num}'/>">
			<input type="text" id="checkin" value="${checkin }" name="checkin">
			~ <input type="text" id="checkout" value="${checkout }"
				name="checkout"> 인원 : <input type="button" id="decrease"
				value="-" onclick="decreaseCnt()"> <input type="number"
				id="rsvnCnt" min="1" value="${cnt }" name="cnt"> <input
				type="button" id="increase" value="+" onclick="increaseCnt()">
	...
	<div id="roomList">
		...
		<c:forEach var="rvo" items="${rlist }">
			<div class="roomDiv">
				<div>
					<span class="roomName"><a
						href="javascript:openRoomDialog(${rvo.room_num})">${rvo.room_type }</a></span><span>(${rvo.room_capa }인실)</span>
				</div>
				<div class="roomImg">
					<img
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div>
						<p>${avo.accom_name }</p>
						<p>
							<span id="roomName"></span><span id="roomCapa"></span>
						</p>
					</div>
					<form method="post" action="<c:url value='/reservation/book'/>">
						<div>
							...
							<input type="hidden" id="room_num" name="room_num"> <input
								type="hidden" id="stay" name="stay"> <input type="text"
								style="width: 100px;" id="rsvnCheckinDatepicker" name="checkin"
								value="${checkin }"> ~ <input type="text"
								style="width: 100px;" id="rsvnCheckoutDatepicker"
								name="checkout" value="${checkout }"> 인원: <select
								id="rsvnDialogCnt" name="cnt">
							</select>
						</div>
						<div>
							<p id="rsvnMsg"></p>
							<div>
								<span>총</span> <span id="roomPrice">0원</span>
							</div>
						</div>
						<div>
