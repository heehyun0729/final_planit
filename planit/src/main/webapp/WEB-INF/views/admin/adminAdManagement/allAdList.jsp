		...
		<div class="tableContentWrapper">
			<div class="tableContentDiv">
				<div class="ui-tabs ui-corner-all ui-widget ui-widget-content">
					...
					<table class="table">
						<tr>
							<th>번호</th><th>신청자</th><th>회사명</th><th>신청일</th><th>결재일</th><th>결제금액</th><th>상세정보</th>
						</tr>
						<c:choose>
					...
					<c:forEach var="i" begin="${map.startPageNum }" end="${map.endPageNum }">
						<c:choose>
							<c:when test="${map.pageNum==i }">
								<a href="${pageContext.request.contextPath }/admin/adminAdManagement/allAdList?pageNum=${i}&field=${param.field}&keyword=${param.keyword}&progress=${map.ad_progress}"><span style="color:black">[${i }]</span></a>
							</c:when>
							<c:otherwise>
								<a href="${pageContext.request.contextPath }/admin/adminAdManagement/allAdList?pageNum=${i}&field=${param.field}&keyword=${param.keyword}&progress=${map.ad_progress}"><span style="color:gray">[${i }]</span></a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<form id="form" method="post" action="<c:url value='/admin/adminAdManagement/allAdList'/>">
						<select name="field">
